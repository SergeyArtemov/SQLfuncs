...
/// . [SQLFuncs].[SQLFuncs.HTTPFunctions].[TryClearMemory] - функция, возвращает разницу междо до- и после- очисточными размерами занятой памяти
...
namespace SQLfuncs
{
    ...
    public class HTTPFunctions
    {
        ...
        public static WebRequest GetRequest(String url) // -- 1.9.1
        {
        ...
        public static SqlBytes ReadBlob(SqlString url) /*1.9.2*///, SqlBoolean NeedLog = SqlBoolean.False
        {
            ...
            {
                byte[] bytes = null;
                WebRequest request = GetRequest(url.Value);
                using (WebResponse response = request.GetResponse())
                {
                    Stream stream = response.GetResponseStream();
                    CommonFunctions.StreamToBytes(ref stream, out bytes);
        ...
        public static SqlBytes HttpGet(SqlString url, SqlString headers) /*1.9.0*/
        {
            ...
            {
                byte[] bytes = null;
                WebRequest req = GetRequest(url.Value);
...
/// <returns></returns>
    ...
    public class TechFunctions 
    {
        ...
        [Microsoft.SqlServer.Server.SqlFunction(DataAccess = DataAccessKind.Read)]
        public static SqlInt64 TryClearMemory()
        {
            //CommonFunctions.SendSQLDebugMessage(String.Format("Start at {0}",DateTime.Now.ToString("HH:mm:ss.ffffff")));
