...
namespace SQLfuncs
{
    ...
    {
        private void AsyncHttpMessageHandler(Object sender, DownloadDataCompletedEventArgs e)
        {


        }

        internal async void AsyncExecuteRequest(string url, string method, string headers, string data, string contenttype, string dbProps)
        {
            // new DownloadDataCompletedEventHandler(AsyncHttpMessageHandler)
            try
            {
                
                using (ByteArrayContent content = new ByteArrayContent(Encoding.UTF8.GetBytes(data)))
                using (HttpClient client = new HttpClient())
                using (HttpRequestMessage msg = new HttpRequestMessage())
                {
                    //CommonFunctions.WriteLog(String.Format("msg add {0}", url));
                    //msg.RequestUri = new Uri(url);
                    //CommonFunctions.WriteLog(String.Format("msg add {0}", method));
                    //msg.Method = new HttpMethod(method);
                    //CommonFunctions.WriteLog(String.Format("msg add {0}", headers));
                    //HTTPFunctions.FillHeaders(headers, msg.Headers);
                    //CommonFunctions.WriteLog(String.Format("msg add {0}", data));
                    //msg.Content = content;
                    //CommonFunctions.WriteLog(String.Format("msg finish"));
                    HTTPFunctions.FillHeaders(headers, msg.Headers);
                    client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue(contenttype));
                    HttpResponseMessage response = await client.PostAsync(url, content);//SendAsync(msg);
                    CommonFunctions.WriteLog(String.Format("{0}", "WAIT"));
                    if (response.IsSuccessStatusCode)
                    {

                        CommonFunctions.WriteLog(String.Format("{0}:{1}", "Step1", response.Content.ToString()));
                    }
                    else
                    {

                        CommonFunctions.WriteLog(String.Format("BOLT!!! : {0}; {1}", response.RequestMessage.ToString(), response.Content.ToString()));
                    }
                }
            }
    ...
    public class HTTPFunctions
    {
        ...
        public static void AsyncReadBlobMethod(SqlString url, SqlString method, SqlString headers, SqlString data, SqlString contenttype, SqlString dbProps) /*1.9.0*/
        {
            ...
            try 
            {
             ...
             AR.AsyncExecuteRequest(url.Value, method.Value, headers.Value, data.Value, contenttype.Value, dbProps.Value);
