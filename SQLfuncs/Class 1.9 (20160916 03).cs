...
namespace SQLfuncs
{
    ...
    class RequestResult
    {
        ...
        public RequestResult(int aCode, string aMessage)
        {
            this.resCode = aCode;
            this.resMessage= aMessage;
        }
    }
    ...
    public class AsyncRequest 
    {
        ...
        internal RequestResult RR(string url, string method) 
        {
           ...
           StringBuilder sb = new StringBuilder(String.Format("{0}\r\n", ""));
            ...
            CommonFunctions.WriteLog(String.Format("RR: {0}", result.resMessage));
            return result;
        }

        delegate Action<RequestData> ActionRequest(RequestData data);

        private void Starter(RequestData data) 
        {
            CommonFunctions.WriteLog("Starter start");
            RequestResult res = RR(data.url, data.method);
            //Task.WaitAll();
            CommonFunctions.WriteLog("Starter end");
        }
        ...
        internal async void AsyncCaller(string url, string method, string headers, string data, string contenttype, string dbProps)
        {
            ...
            try
            {
                Action<RequestData> action = Starter;// (new RequestData(url, method));
                await Task.Run(()=>Starter(new RequestData(url, method)));
                //Task.WaitAll();
                //await Task.Run();

                //Task<RequestResult> res = Task<RequestResult>.Factory.StartNew(RR);
                //res.Wait();
                CommonFunctions.WriteLog("AsyncCaller: {0}, {1}");
            }
