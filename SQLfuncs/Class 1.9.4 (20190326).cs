...
/// <remarks> ВНИМАНИЕ! Смысла в асинхронных методах в этой библиотеке нет: MSSQL завершает работу функций(процедур) не замечая работающих CallBack-ов</remarks>
// SQLfuncs20190325  - пароль подписывания (SQLfuncs.pfx)

// вот тут документация на CLR 
// https://docs.microsoft.com/ru-ru/sql/relational-databases/clr-integration/common-language-runtime-integration-overview?view=sql-server-2016


/// <version_info>
/// /// Новое для 1.9.4 на 20190326 -------------------------------------------------
/// . [SQLFuncs].[SQLFuncs.HTTPFunctions].[TryClearMemoryFunc] - функция, возвращает разницу междо до- и после- очисточными размерами занятой памяти
/// /// Новое для 1.9.3 на 20170609 -------------------------------------------------
/// . [SQLFuncs].[SQLFuncs.HTTPFunctions].[ReadBlobMethodEx] - изменена (добавлено поле "header" (заголовок ответа))
/// Новое для 1.9.2 на 20161004 -------------------------------------------------
/// . [SQLFuncs].[SQLFuncs.TechFunctions].[WriteLog] добавлена
/// . [SQLFuncs].[SQLFuncs.HTTPFunctions].[ReadBlob] - изменена (возврат NULL по ошибке)
/// Новое для 1.9.1 на 20160930 -------------------------------------------------
/// . [SQLFuncs].[SQLFuncs.HTTPFunctions].[HttpGetBinary] восстановлена по коду assembly HttpGetBlob 
/// . [SQLFuncs].[SQLFuncs.HTTPFunctions].[GetResponse] доавлена
/// . [SQLFuncs].[SQLFuncs.HTTPFunctions].[GetRequest] добавлена
/// . [SQLFuncs].[SQLFuncs.HTTPFunctions].[ReadBlob] - изменена (WebRequest через GetRequest)
/// . [SQLFuncs].[SQLFuncs.HTTPFunctions].[HttpGet] - изменена (WebRequest через GetRequest)
/// . [SQLFuncs].[SQLFuncs.HTTPFunctions].[ReadBlobMethod] - изменена (WebRequest через GetRequest)
/// . [SQLFuncs].[SQLFuncs.HTTPFunctions].[ReadBlobMethodEx] - изменена (WebRequest через GetRequest)
...
namespace SQLfuncs
{
        ...
        public static string GetLogFileName()
        {
            ...
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection("context connection=true"))
                {
                    ...
                    {
                        if (reader.Read())
                            res.AppendFormat("{0}{1}.{2}({3}).txt", @"C:\tmp\", NormalizeFileName(String.Format("{0}.{1}", reader.GetSqlString(0).Value, reader.GetSqlString(1).Value)), DateTime.Now.ToString("yyyMMdd"),DayPart);
                        else
            ...
            catch { return String.Format("{0}{1}.txt", @"C:\tmp\", DateTime.Now.ToString("yyyyMMdd")); } //-- обработка без наличия подключения (для локальных приложений и т.п.)
        }

        public static void WriteLog(string aMessage, bool aNoCRLF = false)
        {
            #if (FREEMEM)
            GC.Collect();
            #endif
    ...
    public class HTTPFunctions
    {
        ...
        public static int _timeout = 10000; // -- 10 seconds
        ...
        public static byte[] GetResponse(WebRequest request) // -- 1.9.1
        {
            byte[] bytes = null;
            try
            {
        ...
        public static WebRequest GetRequest(ref String url) // -- 1.9.1
        {
            Uri urlCheck = new Uri(url);
            WebRequest request = null; // -- https://msdn.microsoft.com/ru-ru/library/system.net.webrequest(v=vs.110).aspx
            if (urlCheck.IsFile)
                {request = (FileWebRequest)WebRequest.Create(url);} 
            else{
                if ("ftp" == urlCheck.Scheme)
        ...
        // -- из прошлого см. примечания снизу для WebClient ------
        [Microsoft.SqlServer.Server.SqlFunction(DataAccess = DataAccessKind.Read)]
        public static SqlBytes HttpGetBinary(SqlString url)
        {
            // -- старый рабочий код, тестировать на файле на сетевом ресурсе "\\CALL2.KUPIVIP.LOCAL\WavsArchive\20160924\55305699_184431.mp3"
        ...
        [Microsoft.SqlServer.Server.SqlFunction(DataAccess = DataAccessKind.Read)]
        public static SqlBytes ReadBlob(SqlString url) /*1.9.2*///, SqlBoolean NeedLog = SqlBoolean.False
        {
            #if (FREEMEM)
            GC.Collect();
            #endif
            ...
            {
                byte[] bytes = null;
                string _url = url.Value;
                WebRequest request = GetRequest(ref _url);
                using (WebResponse response = request.GetResponse())
                {
                    Stream stream = response.GetResponseStream();
                    CommonFunctions.StreamToBytes(ref stream, out bytes);
            ...
            {// -- 1.9.2 20161007
                CommonFunctions.WriteLog(String.Format("{0}: {1}",E.Message,url.Value));
                return null;
            }
            return blob;
        }
        ...
        public static SqlBytes HttpGet(SqlString url, SqlString headers) /*1.9.0*/
        {
            ...
            try
            {
                byte[] bytes = null;
                string _url = url.Value;
                WebRequest req = GetRequest(ref _url);
                req.Timeout = _timeout;
                if (req is HttpWebRequest)
                {
                    (req as HttpWebRequest).ContentType = "";
                    (req as HttpWebRequest).Method = "GET";
                    (req as HttpWebRequest).Accept = "text/html, application/xhtml+xml, */*";
                    (req as HttpWebRequest).UserAgent = "Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.2; .NET CLR 1.0.3705;)";
                }
                else
...
/// <returns></returns>
        ...
        public static SqlBytes ReadBlobMethod(SqlString url, SqlString Method, SqlString headers, SqlString parameters, SqlString contenttype) /*1.9.0*/
        {
            ...
            try
            {
                ...
                if (req is HttpWebRequest)
                {
                    ...
                    (req as HttpWebRequest).UserAgent = "Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.2; .NET CLR 1.0.3705;)";
                    (req as HttpWebRequest).ContentType = "";
                    (req as HttpWebRequest).Method = Method.Value;
                    if (contenttype.IsNull == true)
                    { (req as HttpWebRequest).ContentType = ""; }
                    else
                    { (req as HttpWebRequest).ContentType = contenttype.Value; }
                    FillHeaders(headers.Value, (req as HttpWebRequest).Headers);
                    if ((req as HttpWebRequest).Method.ToUpper() != "GET")
                    {
                        byte[] sentData = Encoding.GetEncoding(1251).GetBytes(HttpData);
                        req.ContentLength = sentData.Length;
                        using (Stream sendStream = (req as HttpWebRequest).GetRequestStream())
                        {
        ...
        private class ReadBlobMethodExsResult
        {
            ...
            public SqlString Header;

            public ReadBlobMethodExsResult(SqlBytes result, SqlString debug, SqlString header)
            {
                Result = result;
                Debug = debug;
                Header = header;
        ...
        public static void ReadBlobMethodEx_FillRow(object resultCollection, out SqlBytes result, out SqlString debug, out SqlString header)
        {
            ReadBlobMethodExsResult OneRecord = (ReadBlobMethodExsResult)resultCollection;
            result = OneRecord.Result;
            debug = OneRecord.Debug;
            header = OneRecord.Header;
        }

        [Microsoft.SqlServer.Server.SqlFunction(DataAccess = DataAccessKind.Read
                                              , FillRowMethodName = "ReadBlobMethodEx_FillRow"
                                              , TableDefinition = "result nvarchar(max), debug nvarchar(max), header nvarchar(max)")]
        public static IEnumerable ReadBlobMethodEx(SqlString url, SqlString Method, SqlString headers, SqlString parameters, SqlString contenttype) /*1.9.0*/
        {
            ...
            try
            {
                ...
                if (req is HttpWebRequest)
                {
                    ...
                    if ((req as HttpWebRequest).Method.ToUpper() != "GET")
                    {
                        byte[] sentData = Encoding.GetEncoding(1251).GetBytes(HttpData);
                        req.ContentLength = sentData.Length;
                        Stream sendStream = (req as HttpWebRequest).GetRequestStream();
                        sendStream.Write(sentData, 0, sentData.Length);
                        sentData = null;
                    }
                }
                ...
                headerSB.Append(res.Headers.ToString());
                byte[] bytes = null;
                Stream ReceiveStream = res.GetResponseStream();
                CommonFunctions.StreamToBytes(ref ReceiveStream, out bytes);
                ReceiveStream = null;
            ...
            resultCollection.Add(new ReadBlobMethodExsResult(blob, debugSB.ToString(), headerSB.ToString()));
    ...
    public class TechFunctions 
    {
        ...
        [Microsoft.SqlServer.Server.SqlFunction(DataAccess = DataAccessKind.Read)]
        public static SqlInt64 TryClearMemoryFunc()
        {
            SqlInt64 memsize = GC.GetTotalMemory(false);
            GC.Collect();
            return memsize - GC.GetTotalMemory(true);
        }

        [Microsoft.SqlServer.Server.SqlFunction(DataAccess = DataAccessKind.Read)]
        public static void WriteLog(SqlString Message)
        {
            if ((Message.IsNull) || ("" == Message.Value)) { return; }
            CommonFunctions.WriteLog(Message.Value);
        }
        
        // -- аналог для использования в функциях ------------------
        [Microsoft.SqlServer.Server.SqlFunction(DataAccess = DataAccessKind.Read)]
        public static bool WriteLogFunc(SqlString Message)
        {
            if ((Message.IsNull) || ("" == Message.Value)) { return false; }
            CommonFunctions.WriteLog(Message.Value);
            return true;
        } 
