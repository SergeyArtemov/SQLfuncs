...
namespace SQLfuncs
{
    ...
    public class ReadWriteBlob
    {
        ...
        [Microsoft.SqlServer.Server.SqlFunction(DataAccess = DataAccessKind.Read)]
        public static SqlBytes ReadBlob_OLD(SqlString url)
        {
            string HttpUrl = url.Value;
            Uri urlCheck = new Uri(HttpUrl);
            ...
            byte[] res = new byte[ms.Length];
            res = ms.ToArray();
            SqlBytes blob = new SqlBytes(res);
            data.Close();
            data.Dispose();
            data = null;
            ms.Close();
    ...
    public class ReadWriteBlobPost
    {
        ...
        // ---- with "Dispose()" and "Close()" metods ----------------------
        [Microsoft.SqlServer.Server.SqlFunction(DataAccess = DataAccessKind.Read)]
        public static SqlBytes ReadBlobPost_OLD(SqlString url, SqlString parameters, SqlString contenttype)
        {
            string HttpUrl = url.Value;
            string HttpData = parameters.Value;
            WebRequest req = WebRequest.Create(HttpUrl);
