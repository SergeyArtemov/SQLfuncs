...
/// Новое для 1.9.0 на 20160906 -------------------------------------------------
/// 1. Перенесен прямой вызов сборщика мусора в начало процедур и фунций (паранойя, эмпирика на KUPIVIP00000\KHS_SQLSERVER.TEST via ProcessExplorer)
/// 2. ReadWriteBlobPost.ReadBlobMethodEx (возвращает таблицу)
/// 3. [SQLFuncs].[SQLFuncs.TestFunctions].[TableReturnFunction] (тестовая таблица с разнородными данными)
/// 4. [SQLFuncs].[SQLFuncs.StringFunctions].[UnicodeToUTF8] !удалена 
/// 5. [SQLFuncs].[SQLFuncs.StringFunctions].[UTF8ToUnicode] !удалена
/// 6. [SQLFuncs].[SQLFuncs.StringFunctions].[AsWin1251] добавлена (аналог UnicodeToUTF8 на её замену в дальнейшем)
/// 7. [SQLFuncs].[SQLFuncs.StringFunctions].[AsUTF8] добавлена (аналог UTF8ToUnicode на её замену в дальнейшем)
/// . [SQLFuncs].[SQLFuncs.Emarsys.Send] - изменена
/// . [SQLFuncs].[SQLFuncs.ReadWriteBlob.ReadBlob] - изменена
/// . [SQLFuncs].[SQLFuncs.ReadWriteBlob.ReadBlobEx] - изменена
/// . [SQLFuncs].[SQLFuncs.ReadWriteBlob.HttpGet] - изменена
/// . [SQLFuncs].[SQLFuncs.ReadWriteBlobPost.ReadBlobPost] - изменена
/// . [SQLFuncs].[SQLFuncs.ReadWriteBlobPost.ReadBlobPostEx] - изменена
/// . [SQLFuncs].[SQLFuncs.ReadWriteBlobPost.ReadBlobMethod] - изменена
/// . [SQLFuncs].[SQLFuncs.ReadWriteBlobPost.ReadBlobMethodEx] - изменена
    ...
    class Emarsys
    {
        ...
        public byte[] Send(string method, string uri, string postData, out string err) /*1.9.0*/
        {
            ...
            try
            {
                ...
                string passwordDigest = System.Convert.ToBase64String(Encoding.UTF8.GetBytes(Sha1(nonce + timestamp + secret)));
...
// -- end of "Levis" class -----------------------------------------------------------------------------------------------------------
    ...
    public class ReadWriteBlob
    {
        ...
        public static SqlBytes ReadBlob(SqlString url) /*1.9.0*/
        {
            GC.Collect();
            SqlBytes blob = null;
            ...
            try
            {
                ...
                using (WebClient wc = new WebClient())
                {
                    ...
                    byte[] bytes = null;
                    Stream ReceiveStream = wc.OpenRead(urlCheck);
                    CommonFunctions.StreamToBytes(ref ReceiveStream, out bytes);
                    ReceiveStream = null;
        ...
        // -- old ------
        [Microsoft.SqlServer.Server.SqlFunction(DataAccess = DataAccessKind.Read)]
        public static SqlBytes ReadBlobEx(SqlString url, SqlString headers) /*1.9.0*/
        {
            GC.Collect();
            SqlBytes blob = null;
    ...
    public class ReadWriteBlobPost
    {
        ...
        [Microsoft.SqlServer.Server.SqlFunction(DataAccess = DataAccessKind.Read)]
        public static SqlBytes ReadBlobPost(SqlString url, SqlString parameters, SqlString contenttype) /*1.9.0*/
        {
            GC.Collect();
            SqlBytes blob = null;
            if (url.IsNull == true) { return blob; }
        ...
        [Microsoft.SqlServer.Server.SqlFunction(DataAccess = DataAccessKind.Read)]
        public static SqlBytes ReadBlobPostEx(SqlString url, SqlString headers, SqlString parameters, SqlString contenttype) /*1.9.0*/
        {
            GC.Collect();
            SqlBytes blob = null;
            if (url.IsNull == true) { return blob; }
    ...
    public class StringFunctions 
    {
        ...
        [Microsoft.SqlServer.Server.SqlFunction(DataAccess = DataAccessKind.Read)] 
        public static SqlString AsWin1251(SqlString source)
        {
            GC.Collect();
            if (source.IsNull)
                return ""; 
        ...
        [Microsoft.SqlServer.Server.SqlFunction(DataAccess = DataAccessKind.Read)]
        public static SqlString AsUTF8(SqlString source) 
        {
            GC.Collect();
            if (source.IsNull) 
                return ""; 
                ...
                using (MemoryStream mem = new MemoryStream(Data.Value))
                {
                    using (GZipStream gz = new GZipStream(mem, CompressionMode.Decompress))
                    {
                        using (var ms = new MemoryStream())
                        {
                          ...
                          int buffsize = 262144;//256k 1048576;//1024k(1M)
