...
if not Exists(SELECT * FROM  sys.assemblies where Name='System.Runtime.Serialization')
   ...
   END TRY
   BEGIN CATCH
   print '--> сборка System.Runtime.Serialization идентична предыдущей и не требует обновлени€'
   END CATCH 
GO
...
if not Exists(SELECT * FROM  sys.assemblies where Name='SQLfuncs.XmlSerializers')
   ...
   END TRY
   BEGIN CATCH
   print '--> сборка SQLfuncs.XmlSerializers идентична предыдущей и не требует обновлени€'
   END CATCH
GO
